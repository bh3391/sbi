  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (navigator.share) {
        ...
        alert(
          "Fitur share tidak didukung di browser ini, silakan gunakan tombol download.",
        );
      }
    } catch (err) {
        ...
        >
          ...
          <div ref={receiptRef} className="bg-white p-4">
            ...
            <div className="space-y-4 border-y border-dashed border-slate-200 py-6 mb-8">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-800 font-black uppercase text-right">
                  {studentName}
                </span>
              </div>
              <div className="flex justify-between text-xs">
            ...
            <div className="bg-emerald-50 p-4 rounded-2xl flex items-start gap-3 border border-emerald-100 mb-8">
              ...
              <p className="text-[10px] text-emerald-700 font-bold leading-relaxed uppercase tracking-tight">
                Pembayaran ini telah terverifikasi secara sah oleh sistem
                manajemen Bimbel Pro.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
            >
              <Download size={14} /> Simpan PNG
            </button>
            <button
              onClick={handleShare}
              className="flex-[0.5] bg-slate-100 text-slate-600 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
            >
              <Share2 size={14} />
            </button>
          </div>
