...
export default function PaymentTable({ initialData }: { initialData: any[] }) {
  ...
  const filteredData = useMemo(() => {
    return initialData.filter((payment) => {
      // PERBAIKAN: Gunakan 'students' (jamak) dan map semua nama untuk pencarian
      const studentNames =
        payment.students
          ?.map((s: any) => `${s.fullName} ${s.nickname}`)
          .join(" ") || "";
      const searchTarget =
        `${studentNames} ${payment.category} ${payment.notes || ""}`.toLowerCase();

      const matchSearch = searchTarget.includes(searchTerm.toLowerCase());
      const matchStatus =
        statusFilter === "ALL" ? true : payment.status === statusFilter;
      const matchCategory =
        categoryFilter === "ALL" ? true : payment.category === categoryFilter;
  ...
  if (!initialData || initialData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 mt-6 bg-gradient-to-br from-cyan-50 to-fuchsia-50 rounded-2xl border border-dashed border-cyan-200">
        ...
        <h3 className="text-cyan-700 font-black uppercase tracking-tight text-sm">
          Belum Ada Pembayaran
        </h3>
        <p className="text-cyan-400 text-[10px] text-center mt-1">
          Silahkan klik tombol + untuk menginput data.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full mt-2 max-w-3xl mx-auto px-1 space-y-4">
      ...
      <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md p-3 rounded-2xl border border-cyan-100 shadow-sm space-y-3">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
          {["ALL", "PENDING", "SUCCESS"].map((s) => (
            ...
            >
              {s === "ALL" && <LayoutGrid size={10} className="inline mr-1" />}
              {s}
            </button>
          ))}
      ...
      <div className="space-y-2 pb-20">
        {filteredData.slice(0, displayLimit).map((payment) => {
          // Logic untuk penamaan (mendukung grup)
          const studentCount = payment.students?.length || 0;
          const mainStudent = payment.students?.[0];
          const displayName =
            studentCount > 1
              ? `${mainStudent?.nickname || mainStudent?.fullName} + ${studentCount - 1} Anak`
              : mainStudent?.nickname || mainStudent?.fullName || "No Student";
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <div className="flex-1">
                  <div className="flex items-center gap-1 mb-0.5">
                    ...
                    {studentCount > 1 && (
                      <span className="bg-indigo-100 text-indigo-700 text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider flex items-center gap-0.5">
                        <Users size={8} /> GROUP
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-0.5 items-end">
                  ...
                  {payment.notes && (
                    <span className="text-[7px] text-slate-400 font-bold uppercase truncate max-w-[100px]">
                      {payment.notes}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between border-t border-cyan-100 pt-2 mt-1">
                <div>
                  <p className="text-[8px] text-cyan-500 uppercase font-black tracking-[0.15em]">
                    Nominal
                  </p>
                  ...
                  <p
                    className={`text-[7px] font-bold uppercase tracking-widest mt-1 ${payment.createdById ? "text-amber-500" : "text-rose-400"}`}
                  >
                    by {payment.createdBy?.nickname || "System"}
                  </p>
                </div>
                ...
                {payment.status === "PENDING" ? (
                  <button
                    onClick={async () => {
                      if (
                        confirm(`Konfirmasi pembayaran untuk ${displayName}?`)
                      ) {
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[7px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                      By {payment.verifiedBy?.nickname || "Admin"}
                    </p>
                  </div>
                )}
              </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${currentStyle}`}
    >
