...
export default function StudentProgressChart({
  ...
  const data = [...attendances].reverse().map((atd) => ({
    tanggal: new Date(atd.date).toLocaleDateString("id-ID", {
      day: "2-digit",
      month: "short",
    }),
    ...
    <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-[9px] text-fuchsia-500 font-bold italic">
            Berdasarkan skor pertemuan terakhir
          </p>
        </div>
      </div>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-slate-900 text-white p-3 rounded-2xl shadow-xl border border-white/10">
                      ...
                      <p className="text-sm font-black text-fuchsia-400">
                        Nilai: {payload[0].payload.skorHuruf}
                      </p>
                      <p className="text-[9px] font-medium leading-tight mt-1">
                        {payload[0].payload.materi}
                      </p>
                    </div>
                  );
                }
      ...
      <div className="mt-4 flex justify-between px-2">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] font-black text-fuchsia-600 uppercase italic">
            Grade A
          </p>
        </div>
      </div>
