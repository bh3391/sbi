"use client"
import { Plus, X, Wallet, Calendar, User, FileText, Search, Banknote, Landmark } from "lucide-react";
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4">
            ...
            >
              <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 p-5 text-white relative">
                <button onClick={() => setIsOpen(false)} className="absolute right-3 top-3 bg-white/20 p-0.5 rounded-full hover:bg-white/40"><X size={14} /></button>
                <h2 className="text-sm font-black uppercase tracking-wider">Input Pembayaran</h2>
                <p className="text-[9px] opacity-80">Pilih metode CASH untuk aktivasi sesi instan</p>
              </div>
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <Search className="absolute left-3 top-2.5 text-slate-300 group-focus-within:text-cyan-500 transition-colors" size={12} />
                    <input 
                      type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Metode Pembayaran</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Kategori</label>
                    <select 
                      name="category" 
                      value={category}
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold uppercase text-slate-400 ml-1 flex items-center gap-1"><Wallet size={10} /> Nominal</label>
                    <input 
                      name="amount" 
                      type="number" 
                      value={amount}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold uppercase text-slate-400 ml-1 flex items-center gap-1"><Calendar size={10} /> Untuk Bulan</label>
                    <select name="month" defaultValue={new Date().getMonth() + 1} className="w-full p-2.5 bg-white rounded-xl ring-1 ring-slate-200 text-[11px] outline-none">
                      {months.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Tahun</label>
                    <input name="year" type="number" defaultValue={new Date().getFullYear()} className="w-full p-2.5 bg-white rounded-xl ring-1 ring-slate-200 text-[11px] outline-none" />
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] font-bold uppercase text-slate-400 ml-1 flex items-center gap-1"><FileText size={10} /> Keterangan Tambahan</label>
                  <textarea name="notes" rows={2} placeholder="Misal: Lunas via transfer bank..." className="w-full p-2.5 bg-white rounded-xl ring-1 ring-slate-200 outline-none text-[11px]" />
                </div>
