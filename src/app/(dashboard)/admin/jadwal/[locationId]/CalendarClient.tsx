...
export default function CalendarClient({
  ...
  const getSubjectColor = (name: string) => {
    ...
    if (lowerName.includes("calistung"))
      return "bg-fuchsia-100 text-fuchsia-600 border-fuchsia-200";
    if (lowerName.includes("inggris"))
        ...
        <div className="mb-2 px-1">
          <div className="relative group">
            ...
            <div className="flex justify-between px-6 mt-2">
              ...
              {rooms[roomIdx]?.schedules?.filter(
                (s: any) => s.day === selectedDay,
              ).length > 0 && (
        ...
        <AnimatePresence mode="wait">
          ...
          >
                  ...
                  <div className="flex items-center justify-between bg-cyan-50/50 px-4 py-3 border-b border-slate-100">
                    <div className="flex items-center gap-2 text-slate-500">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest">
                        {sch.session.name}: {sch.session.startTime} -{" "}
                        {sch.session.endTime}
                      </span>
                    </div>
                    <span
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center gap-3 mb-5">
                      <div className="h-9 w-9 bg-cyan-500 rounded-xl flex items-center justify-center text-white shadow-lg">
                        <img
                          src={sch.teacher.image || "/default-avatar.png"}
                          alt={sch.teacher.nickname || sch.teacher.name}
                      ...
                      <div className="justify-end text-right ml-auto">
                        <span
                          className={`text-[10px]  rounded-full p-1  ${getSubjectColor(sch.subject?.name || "")}`}
                        >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {sch.students.map((std: any) => (
                        <div
                          key={std.id}
                          className="px-4 py-2 bg-slate-50 rounded-xl text-[10px] font-bold text-slate-600 border border-slate-100 uppercase flex items-center gap-2 shadow-sm"
                        >
                      ...
                      {sch.students.length < 5 && (
                        <button
                          ...
                          className="px-4 py-2 bg-white border border-dashed border-cyan-300 rounded-xl text-[10px] font-black text-cyan-600 uppercase flex items-center gap-2 hover:bg-cyan-50 transition-all active:scale-95"
                        >
            ...
            <button
              ...
              className="w-full bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-10 flex flex-col items-center justify-center group hover:border-fuchsia-400 transition-all active:scale-95 shadow-sm"
            >
