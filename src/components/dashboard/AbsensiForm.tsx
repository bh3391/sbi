...
export default function AbsensiForm({
  ...
  const handleSave = async () => {
    ...
    const isBasicValid = rows.every(
      (r) => r.studentId && r.subjectId && r.sessionId,
    );
    if (!isBasicValid) {
    ...
    if (incompleteAddon) {
      ...
      toast.error(
        `Kategori Add-on untuk ${student?.nickname || "siswa"} belum dipilih!`,
      );
      return;
    }
      ...
      >
        ...
        <header className="px-4 py-3 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-cyan-50 z-30">
          <div className="flex flex-col">
            <h2 className="text-[11px] font-bold uppercase tracking-tight text-slate-800">
          ...
          <button
            onClick={onClose}
            className="p-1.5 bg-fuchsia-500 text-white rounded-full active:scale-90"
          >
            <X size={16} />
          </button>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-4 py-4 space-y-6 pb-32">
          {rows.map((row, index) => (
            <div
              key={row.id}
              className="relative p-3 rounded-xl border border-fuchsia-500 bg-fuchsia-50/30 space-y-3"
            >
              <div className="flex justify-between items-center border-b border-fuchsia-100 pb-2">
                <span className="text-[9px] font-bold text-fuchsia-600 uppercase tracking-widest">
                  Siswa Ke-{index + 1}
                </span>
                {rows.length > 1 && (
                  <button
              ...
              <div className="space-y-1 relative">
                <label className="text-[8px] font-bold text-slate-700 uppercase tracking-widest ml-1">
                  Nama Siswa
                </label>
                <button
                  type="button"
                  onClick={() =>
                    setOpenSearchId(openSearchId === row.id ? null : row.id)
                  }
                ...
                >
                  <span
                    className={
                      row.studentId ? "text-slate-800" : "text-slate-400"
                    }
                  >
                    {row.studentId
                      ? dataSiswa.find((s) => s.id === row.studentId)?.nickname
                      : "Pilih Siswa..."}
                  </span>
                  <Plus
                    size={14}
                    className={openSearchId === row.id ? "rotate-45" : ""}
                  />
                </button>
                      ...
                      {dataSiswa
                        ...
                        .map((s) => (
                          <button
                            key={s.id}
                            onClick={() => {
                              updateRow(row.id, "studentId", s.id);
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-[8px] font-bold text-slate-700 uppercase">
                    Subject
                  </label>
                  <select
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={row.sessionId}
                    onChange={(e) =>
                      updateRow(row.id, "sessionId", e.target.value)
                    }
                    className="w-full bg-white border border-slate-200 rounded-lg py-2 text-[10px] font-bold outline-none"
                  >
                    <option value="">Pilih...</option>
                    {dataSession.map((ses) => (
              ...
              {row.isAddon && (
                <div className="space-y-1 animate-in slide-in-from-top-1">
                  <label className="text-[8px] font-bold text-cyan-700 uppercase">
                    Kategori Program Add-on
                  </label>
                  <select
                    value={row.addOn}
                  ...
                  >
                    <option value="">
                      Pilih Program (Robotik/Coding/Sains)...
                    </option>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="bg-white border border-slate-200 rounded-lg px-2 text-[10px] font-bold h-9"
                  >
                    {["A", "A-", "B+", "B", "C"].map((sc) => (
                  ...
                  <input
                    ...
                    value={row.materi}
                    onChange={(e) =>
                      updateRow(row.id, "materi", e.target.value)
                    }
          ...
          <button
            type="button"
            onClick={addRow}
            className="w-full py-4 border-2 border-dashed border-slate-200 text-slate-400 text-[9px] font-black uppercase rounded-xl active:bg-slate-50"
          >
            + Tambah Siswa Lagi
          </button>
        </main>
        ...
        <footer className="p-4 border-t border-slate-100 bg-white flex gap-3 sticky bottom-0 z-30 pb-10">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3.5 text-slate-400 text-[9px] font-black uppercase"
          >
            Cancel
          </button>
          <button
            ...
            className={`flex-[2.5] py-3.5 rounded-lg font-black text-[9px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
              isSubmitting
                ...
                : "bg-fuchsia-500 text-white shadow-lg active:scale-95"
            }`}
          >
            {isSubmitting ? (
