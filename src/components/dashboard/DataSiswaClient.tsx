...
<div className="relative inline-block text-left">
  <button 
    onClick={(e) => {
      e.stopPropagation();
      setActiveStatusId(activeStatusId === student.id ? null : student.id);
    }}
  ...
  {userRole !== "TEACHER" && activeStatusId === student.id && (
    <>
      <div className="fixed inset-0 z-50" onClick={() => setActiveStatusId(null)} />
        ...
        {['ACTIVE', 'SUSPEND', 'NEWSTUDENT', 'INACTIVE'].map((st) => (
          ...
          >
            ...
            {st}
                ...
                <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full uppercase ${student.addOnSesi <= 2 ? 'bg-rose-50 text-rose-500' : 'bg-cyan-50 text-cyan-600'}`}>
