...
export default function PaymentTable({ initialData }: { initialData: any[] }) {
  ...
  const filteredData = useMemo(() => {
    return initialData.filter((payment) => {
      ...
      const studentNames = payment.students?.map((s: any) => `${s.fullName} ${s.nickname}`).join(" ") || "";
      const searchTarget = `${studentNames} ${payment.category} ${payment.notes || ""}`.toLowerCase();
    ...
    <div className="w-full mt-2 max-w-3xl mx-auto px-1 space-y-4">
      ...
      <div className="space-y-2 pb-20">
        {filteredData.slice(0, displayLimit).map((payment) => {
          ...
          const displayName = studentCount > 1 
            ...
            >
              ...
              <div className="flex items-center justify-between border-t border-cyan-100 pt-2 mt-1">
                <div>
                  ...
                  <p className={`text-[7px] font-bold uppercase tracking-widest mt-1 ${payment.createdById ? 'text-amber-500' : 'text-rose-400'}`}>
                ...
                {payment.status === "PENDING" ? (
                  <button 
                    onClick={async () => {
                      if(confirm(`Konfirmasi pembayaran untuk ${displayName}?`)) {
