      ...
      >
        ...
        <div className="relative flex justify-center -mt-16 z-10">
          <div className="relative group">
            ...
            <div className="absolute -bottom-2 -right-2">
              <span
                className={`flex items-center gap-1.5 text-[8pt] font-base px-1 py-1.5 rounded-2xl border shadow-lg uppercase tracking-wider text-white ${
                  teacher.role === "ADMIN"
        ...
        <div className="flex-1 overflow-y-auto px-6 pt-4 pb-8 space-y-8 no-scrollbar">
          ...
          <div className="text-center space-y-1">
            <h2 className="text-md font-bold text-slate-800 tracking-tight leading-tight">
              {teacher.name}
            </h2>
            <div className="flex items-center justify-center gap-2">
              <span className="h-px w-4 bg-slate-200"></span>
              <p className="text-[8pt] font-bold text-cyan-600 uppercase tracking-[0.2em]">
                {teacher.nickname || "Staff"}
              </p>
              <span className="h-px w-4 bg-slate-200"></span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-fuchsia-50 border border-slate-100 p-1 rounded-xl flex items-center gap-1">
              <div className="p-1 bg-white rounded-2xl text-fuchsia-500 shadow-sm">
                <Award size={18} />
              </div>
              <div>
                <p className="text-[8pt]  text-slate-400 uppercase tracking-tighter">
                  Subject
                </p>
                <p className="text-[8pt] font-bold text-slate-700 truncate w-20 uppercase">
                  {teacher.specialization || "General"}
                </p>
              </div>
            </div>
            <div className="bg-cyan-50 border border-slate-100 p-1 rounded-xl flex items-center gap-1">
              <div className="p-1 bg-white rounded-2xl text-cyan-500 shadow-sm">
                <MapPin size={18} />
              </div>
              <div>
                <p className="text-[8pt] font-bold text-slate-400 uppercase tracking-tighter">
                  Cabang
                </p>
                <p className="text-[8pt] font-black text-slate-700 truncate w-24 uppercase">
                  {teacher.homebase?.name || "Pusat"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-fuchsia-50 rounded-xl border border-fuchsia-500 p-2 space-y-1">
            {[
              ...
              },
              {
                ...
                label: "ID Guru",
                value: teacher.id.substring(0, 8).toUpperCase(),
              },
            ].map((item, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-3 rounded-2xl hover:bg-pink-50 transition-colors"
              >
                <div className="flex items-center gap-1">
                  <div className="p-1.5 bg-slate-50 rounded-lg">
                    {item.icon}
                  </div>
                  <span className="text-[6pt]  text-slate-700 uppercase tracking-wider">
                    {item.label}
                  </span>
                </div>
                <span className="text-[10pt]  text-slate-700">
                  {item.value}
                </span>
              </div>
            ))}
            ...
            {isResetting ? (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10pt] font-black text-rose-600 uppercase tracking-widest">
                    Keamanan & Password
                  </span>
                </div>
                <div className="text-[10pt] font-black text-rose-400 opacity-0 group-hover:opacity-100 transition-all">
                  GANTI
                </div>
              </button>
            )}
