...
export default function InventoryList({ items, currentUserId }: any) {
  ...
  const handleAdjust = async (id: string, type: "IN" | "OUT", name: string) => {
    ...
    const qty = prompt(
      `Masukkan jumlah ${type === "IN" ? "masuk" : "keluar"} untuk ${name}:`,
    );
  ...
  return (
    <div className="divide-y divide-slate-100 bg-fuchsia-50 rounded-xl shadow border border-slate-100 overflow-hidden">
      {items.map((item: any) => (
        <div
          key={item.id}
          className="p-2 flex items-center justify-between active:bg-cyan-50 transition-colors"
        ...
        >
          ...
          >
            <div className="text-right">
              <span
                className={`text-base font-black ${item.stock < 5 ? "text-red-500" : "text-slate-700"}`}
              >
                {item.stock}
              </span>
              <p className="text-[7px] text-slate-400 uppercase font-bold tracking-tighter">
                Unit
              </p>
            </div>
