...
import { X, AlertCircle, Clock, CheckCircle, Loader2, Search } from "lucide-react";
...
export const StudentAttendanceDrawer = ({ isOpen, onClose, tasks, onOpenLog }: Props) => {
      ...
      <div className="bg-cyan-50 w-full h-[95vh] rounded-t-[2.5rem] flex flex-col animate-in slide-in-from-bottom duration-300 overflow-hidden">
        ...
        <div className="p-6 flex justify-between items-center bg-white/50 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-900 uppercase tracking-tighter">Manajemen Absensi</h2>
              <p className="text-[9px]  text-slate-400 uppercase tracking-[0.2em]">
                {filteredTasks.length} Siswa Ditemukan
              </p>
            </div>
          </div>
          <button onClick={onClose} className="h-10 w-10 bg-fuchsia-500 rounded-full flex items-center justify-center text-white active:scale-90 transition-all">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-6 pb-4 bg-white/50">
          <div className="relative group">
            <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
              <Search size={14} className="text-slate-400 group-focus-within:text-cyan-500 transition-colors" />
            </div>
            <input
              type="text"
        ...
        <div className="flex-1 overflow-y-auto">
          <section className="px-6 py-4 space-y-3 bg-gradient-to-t from-cyan-50 to-white min-h-full">
            <div className="flex items-center gap-2 px-1">
              ...
              <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Daftar Tunggu</h3>
            </div>
                ...
                <tbody className="divide-y divide-rose-50 text-[10px]">
                  {groupedArrays.length > 0 ? (
                    groupedArrays.map((group) => (
                      <tr key={group.student.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-4">
                          ...
                          <span className={`text-[8px] px-2 py-1 rounded-md font-black uppercase tracking-tighter ${
                            group.tasks[0].processStatus === 'LISTED' ? 'bg-rose-50 text-rose-600 border border-rose-100' : 
                            group.tasks[0].processStatus === 'SCHEDULED' ? 'bg-amber-50 text-amber-600 border-amber-100' : 
                      ...
                      <td className="p-12 text-center">
                        <div className="flex flex-col items-center opacity-30">
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase italic tracking-widest">
                            {searchQuery ? "Siswa tidak ditemukan" : "Semua tugas beres!"}
                          </p>
                        </div>
                      </td>
                    </tr>
