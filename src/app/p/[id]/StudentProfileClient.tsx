...
export default function StudentPublicClient({ student }: { student: any }) {
  ...
  const progressPercent = Math.min(
    (student.remainingSesi / totalSesi) * 100,
    100,
  );
        ...
        <section>
          ...
          <div className="space-y-3">
            {student.attendances.length > 0 ? (
              student.attendances.map((atd: any) => (
                <div
                  key={atd.id}
                  className="bg-white p-4 rounded-2xl flex items-center justify-between border border-slate-200/60 shadow-sm"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">
                        Hadir •{" "}
                        {new Date(atd.createdAt).toLocaleTimeString("id-ID", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </p>
            ...
            {student.payments.map((p: any) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p
                    className={`text-[8px] font-bold uppercase tracking-tighter ${p.status === "SUCCESS" ? "text-emerald-600" : "text-amber-500"}`}
                  >
