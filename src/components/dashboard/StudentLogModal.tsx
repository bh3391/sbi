...
export default function StudentLogModal({
  ...
  >({});
  const [activeRescheduleId, setActiveRescheduleId] = useState<string | null>(
    null,
  );
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (activeTab === "PLAN")
      return logs.filter(
        (l) => l.processStatus === "LISTED" || l.processStatus === "SCHEDULED",
      );
    if (activeTab === "DONE")
      return logs.filter((l) => l.processStatus === "DONE");
    return logs;
  }, [logs, activeTab]);
      ...
      <div className="w-full max-w-[320px] bg-cyan-50 h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="px-4 py-3 border-b border-slate-200 flex justify-between items-center bg-cyan-50">
          <div className="flex flex-col">
            ...
            <p className="text-[8px] text-cyan-600 uppercase tracking-widest font-black">
              Log Activity
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 bg-fuchsia-500 rounded-2xl text-slate-50"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/50">
          {isLoading ? (
            // Skeleton Loader...
            [1, 2, 3].map((i) => (
              <div key={i} className="h-12 bg-white animate-pulse rounded-xl" />
            ))
          ) : filteredLogs.length === 0 ? (
            <div className="text-center py-10 text-[9px] font-bold text-slate-400 uppercase">
              No Data
            </div>
          ) : (
              ...
              return (
                <div
                  ...
                  className={`bg-white border transition-all duration-200 overflow-hidden ${
                    isExpanded
                      ? "rounded-2xl border-cyan-200 shadow-md"
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div
                        className={`px-2 py-0.5 rounded text-[7px] font-black uppercase border ${
                        ...
                        {log.rescheduleDate && (
                          <div className="mt-2 p-1.5 bg-amber-50 rounded-lg flex items-center gap-2 text-amber-600">
                            ...
                            <span className="text-[8px] font-black">
                              Reschedule To:{" "}
                              {new Date(log.rescheduleDate).toLocaleDateString(
                                "id-ID",
                              )}
                            </span>
                          </div>
                        ...
                        <div className="space-y-2">
                          {activeRescheduleId === log.id ? (
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() =>
                                  handleStatusUpdate(log.id, "DONE")
                                }
        ...
        <div className="p-4 bg-cyan-50 border-t border-slate-100">
          <button
            onClick={onClose}
            className="w-full py-3 text-[10px] font-black uppercase tracking-widest bg-fuchsia-500 text-white rounded-xl active:scale-95 transition-all"
          >
            Dismiss Panel
          </button>
        </div>
