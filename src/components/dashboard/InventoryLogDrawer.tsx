        ...
        >
          ...
          <div className="p-6">
            <div className="flex justify-between items-start mb-6">
              ...
              <button
                onClick={onClose}
                className="bg-fuchsia-500 p-2 rounded-full text-white"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar">
              {item.logs?.length > 0 ? (
                item.logs.map((log: any) => (
                  <div
                    key={log.id}
                    className="flex gap-4 items-start border-b border-slate-50 pb-2"
                  >
                    <div
                      className={`p-2 rounded-lg ${log.type === "IN" ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
                    >
                      {log.type === "IN" ? (
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`font-black ${log.type === "IN" ? "text-emerald-600" : "text-rose-600"}`}
                        >
                          {log.type === "IN" ? "+" : "-"}
                          {log.quantity}
                        </span>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-[10px] text-slate-400 font-medium uppercase tracking-wider">
                        <span className="flex items-center gap-1">
                          <User size={10} /> {log.createdBy?.name || "System"}
                        </span>
                        <span className="flex items-center gap-1">
                          <Clock size={10} />{" "}
