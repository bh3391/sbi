    ...
    days.forEach((day) => {
      rooms.forEach((room) => {
        const existingSchedules =
          room.schedules?.filter((s: any) => s.day === day) || [];

        sessions.forEach((session) => {
          const scheduleAtThisSession = existingSchedules.find(
              ...
              <div className="flex  justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Tersedia {availableSlots.length} Opsi Jadwal
                  </p>
                </div>
                <button
                  onClick={() => setIsOpen(false)}
                  className="h-8 w-8 bg-fuchsia-500 rounded-2xl flex items-center justify-center text-slate-50"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              {filteredSlots.map((slot) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`h-8 w-8 rounded-2xl flex items-center justify-center shadow-lg ${
                        slot.type === "EMPTY"
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-fuchsia-600 uppercase italic">
                          {slot.day}
                        </span>
                        <span className="text-slate-200 text-[8px]">●</span>
                        <span className="text-[10px] font-black text-cyan-600 uppercase italic">
                          {slot.room.name}
                        </span>
                      </div>
                      ...
                      <h4 className="text-sm font-black text-slate-800 uppercase mt-0.5">
                        {slot.session.name}
                        <span className="text-[9px] font-medium text-slate-400 ml-2 italic">
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        >
                          {slot.type === "EMPTY"
                            ? "0/5"
                            : `${slot.schedule.students.length}/5`}{" "}
                          Murid
                        </div>
                      </div>
                    </div>
