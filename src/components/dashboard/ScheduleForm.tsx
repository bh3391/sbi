...
export default function ScheduleForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          isEditMode ? "Jadwal diperbarui!" : "Jadwal berhasil dibuat!",
        );
        onClose();
      } else {
      ...
      <div className=" w-full max-w-lg bg-cyan-50  p-4 rounded-2xl max-h-[90vh] overflow-y-auto relative border border-slate-100">
        ...
        <div className="flex justify-between  items-center mb-8 border-b border-slate-100">
          <div>
            ...
            <p className="text-[10px] font-bold text-fuchsia-500 uppercase tracking-widest mt-2">
              Ruang {currentRoom.name} • {selectedDay}
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-2 bg-fuchsia-500 hover:bg-slate-50 rounded-full transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-cyan-500 uppercase ml-4">
              Waktu Belajar
            </label>
            <div className="relative">
              <select
                ...
                required
                className="w-full bg-white border border-slate-100 p-4 rounded-2xl text-xs font-bold appearance-none focus:ring-2 focus:ring-fuchsia-100 outline-none"
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                required
                className="w-full bg-white border border-slate-100 p-4 rounded-2xl text-xs font-bold appearance-none outline-none"
              >
            ...
            <div className="flex justify-between items-end px-4">
              <div>
                ...
                <p className="text-[18px] font-bold text-slate-800 leading-none">
                  ...
                  <span className="text-slate-300 text-xs">/ 5 TERPILIH</span>
                </p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center font-black ${isSelected ? "bg-cyan-500 text-white" : "bg-slate-200 text-slate-400"}`}
                        >
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[8px] opacity-50 font-medium mt-1 uppercase tracking-tighter">
                            {std.fullName}
                          </p>
                        </div>
                      </div>
