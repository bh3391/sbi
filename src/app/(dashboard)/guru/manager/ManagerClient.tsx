  ...
  return (
    <div className="pb-24 min-h-screen bg-cyan-50">
      ...
      <div className=" p-2 border-b border-slate-100 shadow-sm sticky top-0 z-30">
        <DashboardHeader userId="" title="Manajemen Master" />
        <div className="flex gap-2 mt-2 p-1 bg-slate-50 rounded-2xl">
          {["lokasi", "sesi", "mapel", "package", "add-on"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === tab ? "bg-cyan-500 text-white shadow-sm" : "text-slate-400"}`}
            >
              {tab}
            </button>
          ))}
      ...
      <div className="p-6">
        <AnimatePresence mode="wait">
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="bg-white border border-slate-200 text-slate-600 py-2 rounded-3xl font-black text-[9px] uppercase tracking-widest flex flex-col items-center gap-2"
                >
                  <Building2 size={20} /> Cabang
                </button>
                <button
                  onClick={() => setModal({ type: "room" })}
                  className="bg-cyan-500 text-white py-2 rounded-3xl font-black text-[9px] uppercase tracking-widest flex flex-col items-center gap-2 shadow-lg shadow-cyan-100"
                >
                  <DoorOpen size={20} /> Ruangan
                </button>
              </div>
                  ...
                  <div
                    ...
                    className={`cursor-pointer transition-all duration-300 bg-white p-2 rounded-2xl border flex items-center justify-between group shadow-sm ${
                      expandedLocation === loc.id
                        ? "border-fuchsia-500 ring-2 ring-fuchsia-50"
                        : "border-fuchsia-300"
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-colors ${
                          expandedLocation === loc.id
                            ...
                            : "bg-slate-50 text-indigo-500 group-hover:bg-cyan-50 group-hover:text-cyan-500"
                        }`}
                      >
                        <MapPin size={24} />
                      </div>
                      <div>
                        <h3 className="font-bold text-slate-800 uppercase leading-none">
                          {loc.name}
                        </h3>
                      ...
                      <ChevronRight
                        size={16}
                        className={`text-slate-300 transition-transform ${expandedLocation === loc.id ? "rotate-90 text-fuchsia-500" : ""}`}
                      />
                    </div>
                  </div>
                        ...
                        {loc.rooms && loc.rooms.length > 0 ? (
                          loc.rooms.map((room: any) => (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleDelete(room.id, room.name, deleteRoom)
                                }
                          ...
                          <p className="text-[9px] text-slate-400 uppercase font-bold pl-4 italic">
          ...
          {activeTab === "sesi" && (
            ...
            >
              <button
                ...
                className="w-full bg-fuchsia-600 text-white py-4 rounded-3xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-fuchsia-100"
              >
              ...
              {initialSessions.map((sesi: any) => (
                <div
                  key={sesi.id}
                  className="bg-white p-2 rounded-lg border border-slate-100 flex items-center justify-between shadow-sm"
                >
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 bg-fuchsia-50 text-fuchsia-500 rounded-xl flex items-center justify-center">
                      <Clock size={20} />
                    </div>
                    <div>
                  ...
                  <button
                    onClick={() =>
                      handleDelete(sesi.id, sesi.name, deleteStudentSession)
                    }
                    className="p-2 text-red-200 hover:text-rose-500"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
                ...
                {initialPackages.map((pkg: any) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(pkg.id, pkg.name, deletePackage)
                        }
                ...
                {initialAddons.map((addon: any) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 mt-1.5 flex items-center gap-2">
                          ...
                          <span className="text-slate-300">•</span>
                          <span className="px-2 py-0.5 bg-slate-100 rounded text-[8px] uppercase">
                            Add-on Sesi
                          </span>
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(addon.id, addon.name, deleteAddon)
                        }
                        className="p-2 text-slate-200 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all"
                        title="Hapus Add-on"
                      >
          ...
          {activeTab === "mapel" && (
            ...
            >
              <button
                onClick={() => setModal({ type: "subject" })}
                className="w-full bg-fuchsia-500 text-white py-4 rounded-3xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-xl"
              >
              ...
              <div className="flex flex-wrap gap-2">
                {initialSubjects.map((sub: any) => (
                  <div
                    key={sub.id}
                    className="px-5 py-3 bg-white border border-slate-100 rounded-2xl flex items-center gap-3 shadow-sm group"
                  >
                    ...
                    <button
                      onClick={() =>
                        handleDelete(sub.id, sub.name, deleteSubject)
                      }
        ...
        {modal.type && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              ...
              {modal.type === "location" && (
                <form
                  onSubmit={(e) => handleAction(e, createLocation)}
                  className="space-y-2"
                >
                  <h2 className="text-lg font-bold text-fuchsia-500 uppercase  mb-4">
                    Tambah Cabang
                  </h2>
                  <input
                    name="name"
                    placeholder="Nama Cabang"
                    className="w-full bg-slate-50 border border-slate-100 px-5 py-4 rounded-2xl text-xs font-bold"
                    required
                  />
                  <input
                    name="address"
                  ...
                  <div className="grid grid-cols-2 gap-2 italic">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-100 px-4 py-3 rounded-2xl text-[10px] font-bold"
                    />
                    <input
                      name="longitude"
                      type="number"
                  ...
                  <button
                    disabled={isPending}
                    className="w-full bg-fuchsia-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase"
                  >
                    {isPending ? (
              ...
              {modal.type === "session" && (
                <form
                  onSubmit={(e) => handleAction(e, createStudentSession)}
                  className="space-y-4"
                >
                  <h2 className="text-xl font-black text-fuchsia-600 uppercase italic mb-4">
                    Buat Slot Waktu
                  </h2>
                  <input
                    name="name"
              ...
              {modal.type === "subject" && (
                <form
                  onSubmit={(e) => handleAction(e, createSubject)}
                  className="space-y-4"
                >
                  <h2 className="text-xl font-black text-slate-900 uppercase italic mb-4">
                    Mata Pelajaran
                  </h2>
                  <input
                    name="name"
                  ...
                  <button
                    disabled={isPending}
                    className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase italic tracking-widest"
                  >
                    Simpan Mapel
                  </button>
                </form>
              ...
              {modal.type === "addon" && (
                <form
                  onSubmit={(e) => handleAction(e, createAddon)}
                  className="space-y-3"
                >
                  {/* Judul dengan skema warna Cyan */}
                  <h2 className="text-xl font-black text-cyan-600 uppercase italic mb-4">
                    Buat Add-on Baru
                  </h2>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-400">
                        Rp
                      </span>
                      <input
                        name="price"
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <input
                        ...
                        required
                      />
                      <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[9px] font-black text-cyan-600 uppercase">
                        Sesi
                      </span>
                    </div>
                    <p className="text-[7px] font-bold text-slate-400 ml-2 mt-1">
                      * Jumlah sesi yang akan didapatkan oleh siswa
                    </p>
                  </div>
              ...
              {modal.type === "addon-edit" && (
                <form
                  onSubmit={(e) =>
                    handleAction(e, (formData: FormData) =>
                      updateAddon(modal.data.id, formData),
                    )
                  }
                  className="space-y-3"
                >
              ...
              {modal.type === "package" && (
                <form
                  onSubmit={(e) => handleAction(e, createPackage)}
                  className="space-y-3"
                >
                  <h2 className="text-xl font-black text-fuchsia-600 uppercase italic mb-4">
                    Buat Paket Kursus
                  ...
                  <button
                    disabled={isPending}
                    className="w-full bg-fuchsia-600 text-white py-4 mt-2 rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-fuchsia-100 transition-all active:scale-95"
                  >
                    {isPending ? (
                      <Loader2 className="animate-spin mx-auto" />
                    ) : (
              ...
              {modal.type === "edit-package" && modal.data && (
                <form
                  onSubmit={(e) =>
                    handleAction(e, (formData: FormData) =>
                      updatePackage(modal.data.id, formData),
                    )
                  }
                  className="space-y-3"
                >
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      name="description"
                      defaultValue={
                        typeof modal.data.description === "object"
                          ? ""
              ...
              {modal.type === "room" && (
                <form
                  onSubmit={(e) => handleAction(e, createRoom)}
                  className="space-y-4"
                >
                  <h2 className="text-xl font-black text-cyan-500 uppercase italic mb-4">
                    Tambah Ruangan
                  </h2>
                  <input
                    name="name"
                    placeholder="Nama Ruang (Contoh: A1)"
                    className="w-full bg-slate-50 border border-slate-100 px-5 py-4 rounded-2xl text-xs font-bold"
                    required
                  />
                  <select
                    name="locationId"
                    className="w-full bg-slate-50 border border-slate-100 px-5 py-4 rounded-2xl text-xs font-bold appearance-none"
                    required
                  >
                  ...
                  <button
                    disabled={isPending}
                    className="w-full bg-cyan-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase"
                  >
                    Simpan Ruang
                  </button>
                </form>
