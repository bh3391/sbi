  ...
  return (
    <div className="min-h-screen bg-cyan-50 pb-32 font-sans antialiased text-slate-900">
      ...
      <main className="max-w-5xl mx-auto p-1 md:p-4 space-y-2">
        ...
        >
          <div className={`w-1.5 h-1.5 rounded-full ${selectedLocation === "ALL" ? "bg-cyan-400 animate-pulse" : "bg-slate-200"}`} />
          All Branches
        </button>
          ...
          {locations.map((loc: any) => {
            ...
            const count = initialStudents.filter((s: any) => s.locationId === loc.id).length;
            ...
            return (
              ...
              >
                <MapPin size={12} className={isActive ? "text-cyan-500" : "text-slate-300"} />
                {loc.name}
                <span className={`ml-1 px-1.5 py-0.5 rounded-lg text-[8px] ${isActive ? "bg-cyan-100 text-cyan-600" : "bg-slate-50 text-slate-400"}`}>
                  {count}
                </span>
              </button>
            );
            ...
            <table className="w-full text-left table-auto">
              <thead>
                <tr className="bg-cyan-50 border-b border-cyan-100">
                  ...
                  <th className="px-4 py-2 text-[8px] font-bold text-slate-800 uppercase tracking-widest">Add-on</th>
                ...
                {filteredStudents.length > 0 ? filteredStudents.map((student: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] font-bold text-slate-600">{student.addOnSesi} Sesi</span>
                          <div className="w-10 h-1 bg-slate-100 rounded-full overflow-hidden">
                            <div 
                              className={`h-full transition-all duration-500 ${student.addOnSesi <= 2 ? 'bg-rose-500' : 'bg-fuchsia-500'}`} 
                              style={{ width: `${Math.min((student.addOnSesi / 12) * 100, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
