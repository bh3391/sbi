    ...
    <div className="fixed inset-0 z-[110] bg-slate-900/40 backdrop-blur-[2px] flex justify-end animate-in fade-in duration-200">
      <div className="w-full max-w-[320px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-cyan-100/50">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-fuchsia-500 rounded-md transition-all text-slate-400"
          >
            <X size={16} />
          </button>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-20 bg-white border border-slate-100 animate-pulse rounded-xl"
                />
              ))}
            </div>
          ) : logs.length === 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full flex flex-col items-center justify-center border shrink-0 ${
                            isLate
                        ...
                        >
                          ...
                          <span className="text-[7px] uppercase font-black tracking-tighter mt-0.5">
                            WIB
                          </span>
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[8px] font-black tracking-widest uppercase border ${
                            isLate
        ...
        <div className="p-3 border-t border-slate-100 bg-white">
          <button
            onClick={onClose}
            className="w-full py-2 text-[9px] font-black uppercase tracking-[0.2em] bg-fuchsia-500 text-white rounded-md"
          >
            Close Panel
          </button>
        </div>
